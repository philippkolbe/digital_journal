...
class JournalPromptWizard extends ConsumerWidget {
  final PersonalityPromptDailyCardObj? dailyCard;

  const JournalPromptWizard({this.dailyCard, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final selectedJournalEntryController =
        ref.watch(selectedJournalEntryProvider.notifier);
    final journalController = ref.watch(journalEntriesProvider.notifier);
    final journalPromptController =
        ref.watch(journalPromptControllerProvider.notifier);
  ...
  Widget _buildPrompt(AsyncValue<ChatMessageObj?> prompt,
    ...
    return AsyncWidget(
      ...
      buildWidget: (prompt) => Row(
        children: [
          Expanded(
              child: Text(
            prompt?.content ?? 'Something went wrong. Please try again.',
            textScaleFactor: 1.2,
          )),
  ...
  void _onPromptAccepted(
    ...
    if (dailyCard != null) {
      ref
          ...
          .updateDailyCard(dailyCard!.copyWith(
            prompt: journalPrompt.valueOrNull?.content,
          ))
          .then((value) =>
              ref.refresh(dailyCardsProviderFamily(dailyCard!.date)));
    }
