...
final StateProvider<PersonalityObj?> selectedPersonalityProvider =
    StateProvider<PersonalityObj?>((ref) => null);

final personalityProviderFamily =
    ProviderFamily<AsyncValue<PersonalityObj>, String>((ref, id) {
...
final personalitiesProvider = StateNotifierProvider<PersonalityController,
  ...
  final selectedPersonalityController =
      ref.watch(selectedPersonalityProvider.notifier);
  ...
  ref.listen(selectedJournalEntryProvider,
      (asyncPrevJournalEntry, asyncJournalEntry) {
    ...
    if (id != null) {
      ref.read(selectedPersonalityProvider.notifier).state =
          controller.getPersonalityFromId(id);
    }
  });
...
class PersonalityController
    extends StateNotifier<AsyncValue<List<PersonalityObj>>> {
  ...
  Future<void> loadPersonalities() async {
    try {
      assert(_userId != null,
          "User must be authenticated to load its (custom) personalities.");
      ...
      final personalities =
          await _personalityRepository.readAllPersonalities(userId: _userId!);
  ...
  Future<PersonalityObj> addPersonality(PersonalityObj personalityObj) async {
    ...
    assert(state is AsyncData,
        "Personalities must be loaded to add new custom personalities.");

    personalityObj = await _personalityRepository
        .createPersonality(personalityObj, userId: _userId!);
  ...
  Future<void> deletePersonality(String entryId) async {
    try {
      assert(state is AsyncData,
          "Personalities must be loaded to delete custom personalities.");

      await _personalityRepository.deletePersonality(entryId, userId: _userId!);

      state =
          AsyncData(state.value!..removeWhere((entry) => entry.id == entryId));
  ...
  Future<void> updatePersonality(PersonalityObj personalityObjUpdate) async {
    try {
      assert(state is AsyncData,
          "Personalities must be loaded to update custom personalities.");
      assert(personalityObjUpdate.id != null,
          "Personality must have an id to update it.");

      final newPersonalityObj = await _personalityRepository
          .updatePersonality(personalityObjUpdate, userId: _userId!);

      state = AsyncData(state.value!
          .map((entry) =>
              entry.id == personalityObjUpdate.id ? newPersonalityObj : entry)
          .toList());
  ...
  PersonalityObj? getPersonalityFromId(String id) {
    return state.valueOrNull
        ?.firstWhereOrNull((personality) => personality.id == id);
  }
}
